...
var spawn_table = {
	"res://Sprites/Enemy/Mite.tscn": 100,	
}
...
# --- NEW: Function for the Boss to call ---
func force_spawn_for_boss(amount: int) -> Array:
	# 1. Start the visual/audio sequence you already built
	trigger_spawn_sequence(amount)
	
	# 2. Wait just a moment for the 'await' in your sequence to finish
	# then return the newly spawned enemies
	await get_tree().create_timer(1.1).timeout 
	
	# Return the mites currently in the group so the King can track them
